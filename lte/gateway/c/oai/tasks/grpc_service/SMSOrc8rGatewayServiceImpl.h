...
class SMSOrc8rGatewayServiceImpl final : public SMSOrc8rGatewayService::Service {
 public:
  ...
  /*
   * Sent from the sms_orc8r to the MME, containing sufficient information
       to allow the paging message to be transmitted.
   *
   * @param context: the grpc Server context
   * @param request: SMOPagingRequest, contains IMSI, VLR name, service indicator,
       and other optional fields
   * @param response (out): Void defined in common.proto
   * @return grpc Status instance
   */
  grpc::Status SMOPagingReq(
      ServerContext* context, const SMOPagingRequest* request,
      Void* response) override;
