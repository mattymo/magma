...
grpc::Status SMSOrc8rGatewayServiceImpl::SMODownlink(
  ...
  std::cout << "[DEBUG] "
            << "Received SMS_ORC8R_DOWNLINK_UNITDATA message from orc8r with IMSI: "
            << itti_msg.imsi << std::endl;
  handle_sms_orc8r_downlink_unitdata(&itti_msg);
  return grpc::Status::OK;
}

grpc::Status SMSOrc8rGatewayServiceImpl::SMOPagingReq(
    ServerContext* context, const SMOPagingRequest* request, Void* response) {
  itti_sgsap_paging_request_t itti_msg;
  convert_proto_msg_to_itti_sgsap_paging_request(request, &itti_msg);
  std::cout << "[DEBUG] Received SMS_ORC8R_PAGING_REQUEST message"
               "from orc8r with IMSI: "
            << itti_msg.imsi << std::endl;
  handle_sms_orc8r_paging_request(&itti_msg);
  return grpc::Status::OK;
}
